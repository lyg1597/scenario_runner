...
class RunStraight(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        loc = carla.Location(x=193.09947204589844, y=-85.6633529663086, z=0.0)
        self._reference_waypoint = self._map.get_waypoint(loc)
        ...
        super(RunStraight, self).__init__("RunStraight",
                                                   ego_vehicles,
                                                   config,
                                                   world,
                                                   debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        self._other_actor_transform = carla.Transform(
            carla.Location(first_vehicle_waypoint.transform.location.x,
                           first_vehicle_waypoint.transform.location.y + 22,
                           first_vehicle_waypoint.transform.location.z + 0.1),
            first_vehicle_waypoint.transform.rotation)
        first_vehicle_transform = carla.Transform(
            carla.Location(self._other_actor_transform.location.x,
                           self._other_actor_transform.location.y + 22,
    ...
    def _create_behavior(self):
        ...
        # actor driving forward at constant speed
        driving_to_next_intersection = py_trees.composites.Parallel("Driving forward at constant speed",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)

        driving_constant_speed = py_trees.composites.Sequence("Start Driving")
        driving_constant_speed.add_child(ChangeAutoPilot(self.other_actors[0], True, 
                                                                     parameters={"max_speed": self._first_vehicle_speed}))
        driving_constant_speed.add_child(KeepVelocity(self.other_actors[0], self._first_vehicle_speed))

        

        # construct scenario
        driving_to_next_intersection.add_child(driving_constant_speed)
